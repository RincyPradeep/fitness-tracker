...
const Profile = () => {
  ...
  const [formData, setFormData] = useState<ProfileFormData>({
    age: 0, weight: 0, height: 0, goal: 'maintain', dailyCalorieIntake: 2000, dailyCalorieBurn: 400 
  })
      ...
      <div className="profile-content">
        ...
        <Card>
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h2 className="text-lg font-semiboldmtext-slate-800 dark:text-white">Your Profile</h2>
              <p className="text-slate-500 dark:text-slate-400 text-xs">
                Memeber since {new Date(user?.createdAt || '').toLocaleDateString()}
              </p>
            </div>
          </div>

          {
            isEditing ? (
              <div className="space-y-4">
                ...
                <Select 
                  label="Fitness Goal"
                  value={formData.goal as string} onChange={(v)=>setFormData({...formData, goal:v as 'lose' | 'maintain' | 'gain'})}
                  options = {goalOptions}
                />
